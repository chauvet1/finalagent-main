...
import { isAuthenticationAvailable, getCurrentTokenInfo, adminAPI, schedulingAPI, sitesAPI, agentsAPI } from '../../services/api';
...
const AdvancedSchedulingPage: React.FC = () => {
  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      const events = shiftsData.map((shift: any) => ({
        id: shift.id,
        title: `${shift.agent?.user?.firstName || shift.agent?.user?.username || 'Unassigned'} - ${shift.site?.name || 'Unknown Site'}`,
      ...
      setError('Failed to load scheduling data. Please check your connection and try again.');
  ...
  const checkScheduleConflicts = async () => {
    ...
    const authAvailable = await isAuthenticationAvailable();
    if (!authAvailable) {
      console.warn('Authentication not available for conflict check');
      setConflicts([]);
      return;
    }
    ...
    try {
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Checking schedule conflicts with ${tokenInfo.type} token`);

      const response = await adminAPI.getScheduleConflicts({
      ...
      // Set empty conflicts as fallback
      setConflicts([]);
    }
  };
  ...
  const handleOptimizeSchedule = async () => {
    ...
    try {
      ...
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Optimizing schedule with ${tokenInfo.type} token`);

      const response = await adminAPI.optimizeSchedule(requirements);
      setOptimizationResults(response.data);
      setOptimizationDialogOpen(true);
    } catch (error) {
      console.error('Failed to optimize schedule:', error);
      setError('Schedule optimization failed. Please check your connection and try again.');
    } finally {
  ...
  const handleApplyOptimization = async () => {
    ...
    try {
      ...
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Applying schedule optimization with ${tokenInfo.type} token`);
      ...
      setError('Failed to apply schedule optimization. Please check your connection and try again.');
    }
  };
  ...
  const handleExportSchedule = async (format: 'csv' | 'pdf' | 'ical') => {
    ...
    try {
      ...
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Exporting schedule with ${tokenInfo.type} token`);

      const response = await adminAPI.exportSchedule(format);
