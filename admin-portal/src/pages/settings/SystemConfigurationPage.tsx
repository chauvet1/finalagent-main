...
import { isAuthenticationAvailable, getCurrentTokenInfo, systemAPI } from '../../services/api';
...
const SystemConfigurationPage: React.FC = () => {
  ...
  const loadSystemConfig = async () => {
    ...
    try {
      ...
      console.debug(`Loading system configuration with ${tokenInfo.type} token`);
      ...
      setError(error.response?.data?.message || error.message || 'Failed to load system configuration');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleSaveConfig = async () => {
    ...
    try {
      ...
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Saving system configuration with ${tokenInfo.type} token`);
      ...
      setError(error.response?.data?.message || error.message || 'Failed to save system configuration');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleTestConnection = async (type: 'email' | 'sms' | 'slack') => {
    ...
    try {
      ...
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Testing ${type} connection with ${tokenInfo.type} token`);
      ...
      setError(`${type.toUpperCase()} connection test failed: ${error.response?.data?.message || error.message}`);
    }
  };
  ...
  const handleBackupSystem = async () => {
    ...
    try {
      ...
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Creating system backup with ${tokenInfo.type} token`);
