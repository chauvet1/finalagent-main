...
const UsersPage: React.FC = () => {
  ...
  const loadUsers = async () => {
    try {
      // Check authentication availability using enhanced token provider
      const authAvailable = await isAuthenticationAvailable();
      if (!authAvailable) {
        console.error('Authentication not available for loading users');
        return;
      }

      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Loading users with ${tokenInfo.type} token`);
  ...
  const handleCreateUser = async (userData: any) => {
    try {
      ...
      const authAvailable = await isAuthenticationAvailable();
      if (!authAvailable) {
        console.error('Authentication not available for creating user');
        return;
      }

      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Creating user with ${tokenInfo.type} token`);
  ...
  const handleEditUser = async (userData: any) => {
    try {
      ...
      const authAvailable = await isAuthenticationAvailable();
      if (!authAvailable) {
        console.error('Authentication not available for updating user');
        return;
      }

      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Updating user with ${tokenInfo.type} token`);
  ...
  const handleDeleteUser = async () => {
    try {
      ...
      const authAvailable = await isAuthenticationAvailable();
      if (!authAvailable) {
        console.error('Authentication not available for deleting user');
        return;
      }

      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Deleting user with ${tokenInfo.type} token`);
