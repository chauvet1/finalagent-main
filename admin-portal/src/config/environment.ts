...
const getPortalUrls = () => {
  ...
  const productionDefaults = {
    client: currentOrigin.replace('/admin', ''), // Remove /admin if present
    admin: currentOrigin,
    api: `${currentOrigin.replace('/admin', '')}/api`
  };
...
const createEnvironmentConfig = (): EnvironmentConfig => {
  ...
  return {
    ...
    app: {
      name: process.env.REACT_APP_NAME || 'BahinLink Admin',
...
export const debugEnvironmentConfig = (): void => {
  ...
  console.group('🌐 Admin Portal Environment Configuration');
...
export const validateEnvironment = (): void => {
  ...
  if (config.isProduction) {
    if (!config.portals.admin) {
      errors.push('Admin portal URL is required in production');
    }
  ...
  if (errors.length > 0) {
    console.error('❌ Admin Portal Environment validation failed:');
    errors.forEach(error => console.error(`  - ${error}`));
    
    if (config.isProduction) {
      throw new Error('Admin Portal environment validation failed in production');
    }
