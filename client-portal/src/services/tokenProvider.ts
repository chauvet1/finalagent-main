...
/**
 * Client portal token provider
 * Handles both Clerk JWT tokens and localStorage-based tokens with development fallback
 */
export class ClientTokenProvider implements TokenProvider {
  ...
  constructor(clerkAuth?: any, config: TokenProviderConfig = {}) {
    ...
    this.config = {
      developmentMode: config.developmentMode !== undefined ? config.developmentMode : process.env.NODE_ENV === 'development',
  ...
  async getAuthToken(): Promise<string> {
    try {
      // First try to get Clerk JWT token if available
      if (this.clerkAuth) {
  ...
  async getTokenInfo(): Promise<TokenInfo> {
    try {
      ...
      // In development mode, provide a development token even when no token is available
      if (this.isDevelopmentMode()) {
  ...
  /**
   * Set token in localStorage (for compatibility with existing system)
   */
  ...
  private getStoredToken(): string | null {
    try {
      ...
      console.debug('Failed to get stored token:', error);
      return null;
    }
  }
  ...
  private getUserEmail(): string {
    try {
      ...
      return this.config.fallbackEmail || 'client@bahinlink.com';
    } catch (error) {
  ...
  /**
   * Check if currently in development mode (dynamic check for tests)
   */
  private isDevelopmentMode(): boolean {
    // Check config first, then fall back to current NODE_ENV
    return this.config.developmentMode || process.env.NODE_ENV === 'development';
  }
...
export function createTokenProvider(clerkAuth?: any, config?: TokenProviderConfig): TokenProvider {
  return new ClientTokenProvider(clerkAuth, config);
}
