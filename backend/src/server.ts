...
// Add missing location endpoints (before authenticated routes)
console.log('Registering custom API endpoints...');

// Legacy endpoint for backward compatibility - redirect to new tracking endpoint
app.get('/api/locations/agents/current', requireAuth, requireAdmin, async (req, res) => {
  console.log('Current agent locations endpoint hit');
  try {
    const { PrismaClient } = await import('@prisma/client');
    const prisma = new PrismaClient();

    // Get current locations of active agents
    ...
    const currentLocations = await Promise.all(
      activeShifts.map(async (shift) => {
        // Get latest location for this agent
        ...
        if (!latestLocation) {
          // Return agent without location if no recent tracking
          return {
            agent: {
              id: shift.agent.id,
...
app.get('/api/analytics/tracking-stats', requireAuth, requireAdmin, async (req, res) => {
...
// Add sites endpoint for tracking map
app.get('/api/sites/tracking', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // Transform sites to include latitude/longitude from coordinates JSON
    const transformedSites = sites.map(site => {
      let latitude = 0, longitude = 0;
      if (site.coordinates && typeof site.coordinates === 'object') {
        const coords = site.coordinates as any;
...
// Add agents endpoint
app.get('/api/agents/active', requireAuth, requireAdmin, async (req, res) => {
...
app.get('/api/analytics/incident-stats', requireAuth, requireAdmin, async (req, res) => {
...
// Add emergency alert management endpoints
app.get('/api/emergency-alerts', requireAuth, async (req, res) => {
  console.log('Emergency alerts endpoint hit');
  try {
    const EmergencyAlertService = (await import('./services/emergencyAlertService')).default;
...
app.post('/api/emergency-alerts/:alertId/acknowledge', requireAuth, async (req, res) => {
...
app.post('/api/emergency-alerts/:alertId/resolve', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // Broadcast resolution to all connected users
    if (wsService) {
...
app.get('/api/emergency-alerts/:alertId', requireAuth, async (req, res) => {
...
app.get('/api/locations/current', requireAuth, requireAdmin, async (req, res) => {
...
app.get('/api/locations/history/:agentId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const locationHistory = await locationService.getLocationHistory(
      agentId,
...
app.get('/api/communications/groups', requireAuth, async (req, res) => {
...
app.get('/api/analytics/communication-stats', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    // Get communication types breakdown
...
// Add POST endpoint for creating communications
app.post('/api/communications', requireAuth, async (req, res) => {
  ...
  try {
    ...
    // For now, use the first agent as sender (in real app, this would come from auth)
    ...
    // Create message recipient if recipientId is provided
    ...
    // If it's a group message, create recipients for all group members
    if (groupId) {
...
app.post('/api/communications/broadcast', async (req, res) => {
  ...
  try {
    ...
    // Create recipients for all users in the specified group or all active users
    ...
    res.status(201).json({
      ...
      data: {
        ...
        recipientCount: recipients.length - 1 // Exclude sender
      },
  ...
  try {
    ...
    const transformedAgents = agents.map(agent => ({
      ...
      performanceMetrics: {
        ...
        attendanceRate: 95, // Could be calculated from attendance records
        punctualityScore: 90, // Could be calculated from shift start times
        incidentReports: 2, // Could be calculated from incident count
        commendations: 3, // Could be calculated from performance reviews
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        metrics: {
          averageShiftDuration: 8, // hours - could be calculated from actual data
          completionRate: 95, // percentage - could be calculated from actual data
          responseTime: 12 // minutes - could be calculated from actual data
        }
      },
      timestamp: new Date().toISOString()
    });
...
app.get('/api/analytics/top-performers', async (req, res) => {
  ...
  try {
    ...
    // Get top performing agents through AgentProfile
...
// Add training management endpoints
...
// Add POST endpoint for creating trainings
...
// Add PUT endpoint for updating trainings
...
// Add PUT endpoint for updating training enrollments
app.put('/api/training-enrollments/:id', async (req, res) => {
  ...
  try {
    ...
    // Calculate progress and completion date based on status
    ...
    if (status === 'COMPLETED') {
      ...
      progress = Math.max(progress, 1); // At least 1% if in progress
...
// Add DELETE endpoint for training enrollments
...
// Add endpoint to get available users for messaging
...
app.use('/api/client-portal', requireAuth, requireClient, clientPortalRouter);
app.use('/api/client', requireAuth, requireClient, clientRouter);
app.use('/api/clients', requireAuth, requireAdmin, clientsRouter);
app.use('/api/admin/contracts', requireAuth, requireAdmin, contractsRouter);
...
// Register geofence routes
app.use('/api', geofenceRouter);

// Temporary simple sites endpoint for debugging
...
try {
  wsService = new WebSocketService(server, redisClient);
