...
const IncidentsScreen: React.FC = () => {
  ...
  const fetchIncidents = useCallback(async () => {
    ...
    try {
      ...
      console.debug(`Loading mobile incidents with ${tokenInfo.type} token`);

      // Use the enhanced mobile API service
      const response = await mobileAPI.getIncidents();
  ...
  const updateIncidentStatus = async (incidentId: string, newStatus: string) => {
    ...
    const authAvailable = await isAuthenticationAvailable();
    if (!authAvailable) {
      Alert.alert('Error', 'Authentication required to update incident status');
      return;
    }
    ...
    try {
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Updating incident status with ${tokenInfo.type} token`);

      // Use the enhanced mobile API service
      const response = await mobileAPI.updateIncident(incidentId, { status: newStatus });
