...
import { isAuthenticationAvailable, getCurrentTokenInfo, mobileAPI } from '../services/api';
...
const DashboardScreen: React.FC = () => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      console.debug(`Loading mobile dashboard with ${tokenInfo.type} token`);

      // Use the enhanced mobile API service
      const dashboardResponse = await mobileAPI.getDashboard();
  ...
  const startShift = async () => {
    ...
    const authAvailable = await isAuthenticationAvailable();
    if (!authAvailable) {
      Alert.alert('Error', 'Authentication required to start shift');
      return;
    }
    ...
    try {
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Starting shift with ${tokenInfo.type} token`);

      // Use the enhanced mobile API service
      const response = await mobileAPI.startShift({});
      ...
      [
        ...
        { 
          ...
          onPress: async () => {
            ...
            const authAvailable = await isAuthenticationAvailable();
            if (!authAvailable) {
              Alert.alert('Error', 'Authentication required to end shift');
              return;
            }
            ...
            try {
              // Get current token info for debugging
              const tokenInfo = await getCurrentTokenInfo();
              console.debug(`Ending shift with ${tokenInfo.type} token`);

              // Use the enhanced mobile API service
              const response = await mobileAPI.endShift(stats?.currentShift?.id || '', {});
  ...
  const takeBreak = async () => {
    ...
    const authAvailable = await isAuthenticationAvailable();
    if (!authAvailable) {
      Alert.alert('Error', 'Authentication required to take break');
      return;
    }
    ...
    try {
      // Get current token info for debugging
      const tokenInfo = await getCurrentTokenInfo();
      console.debug(`Taking break with ${tokenInfo.type} token`);

      // Use the enhanced mobile API service
      const response = await mobileAPI.takeBreak(stats?.currentShift?.id || '');
